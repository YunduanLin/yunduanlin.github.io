...
Assert-NotContains "_data/paper_network.yml" "(?s)id:\s*J1(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*real-time prediction\s*\r?\n|(?s)id:\s*J1(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*crowdsourcing data\s*\r?\n" "J1 should not keep the older prediction/crowdsourcing-data feature wording."
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*J2.*network diffusion.*data-driven optimization.*diffusion model.*user-generated content platform.*short-video platform.*social media.*social network operations.*proprietary data from a short-video platform" "J2 should use the revised network diffusion, method, domain, and data features."
Assert-NotContains "_data/paper_network.yml" "(?s)id:\s*J2(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*diffusion\s*\r?\n|(?s)id:\s*J2(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*optimization\s*\r?\n" "J2 should not keep the older generic diffusion/optimization feature wording."
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*J3.*network diffusion.*network structure.*network centrality.*stochastic modeling.*fixed-point iteration.*network game.*equilibrium.*social network operations.*scalable network operations.*network repository.*Facebook network" "J3 should use the revised network, modeling, equilibrium, domain, and data features."
