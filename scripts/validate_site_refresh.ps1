...
Assert-Contains "_sass/_base.scss" "grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)" "Research stream bibliographies should use two publication columns on desktop."
Assert-Contains "_sass/_base.scss" "(?s)@media \(max-width:\s*900px\).*ol\.bibliography\s*\{[^}]*grid-template-columns:\s*1fr" "Research stream bibliography cards should collapse to one column on smaller screens."
