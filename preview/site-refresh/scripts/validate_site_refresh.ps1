...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*W3.*expert decision.*decision accuracy assessment.*multi-source data.*machine learning.*algorithm design.*design science.*Kaggle.*adult census income.*hospital readmission rates" "W3 should use the revised decision, multi-source-data, method, domain, and data features."
...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*W4.*policy evaluation.*large-scale simulation.*multi-source data.*\r?\n\s*-\s*simulation\s*\r?\n\s*-\s*digital twin.*social media.*social network operations.*user-generated content platform.*AI agent.*scalable network operations.*KuaiRand-Pure random-exposure data.*Tsinghua ShortVideo sampled catalogue.*VK-LSVD" "W4 should use the revised simulation, domain, and multi-source data features."
Assert-NotContains "_data/paper_network.yml" "(?s)id:\s*W4(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*LLM simulation\s*(?=\r?\n|$)|(?s)id:\s*W4(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*digital twin simulation\s*(?=\r?\n|$)|(?s)id:\s*W4(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*content platform governance\s*(?=\r?\n|$)|(?s)id:\s*W4(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*content platforms\s*(?=\r?\n|$)|(?s)id:\s*W4(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*platform operations\s*(?=\r?\n|$)|(?s)id:\s*W4(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*short-video platform data\s*(?=\r?\n|$)" "W4 should not keep older LLM/digital-twin-simulation, content-platform, platform-operations, or old short-video data feature wording."
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*W5.*behavioral simulation.*platform decision support.*LLM simulation.*\r?\n\s*-\s*survey\s*\r?\n.*transportation system" "W5 should use the revised LLM-simulation concept, survey method, and transportation-system domain."
Assert-NotContains "_data/paper_network.yml" "(?s)id:\s*W5(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*agent-based simulation\s*(?=\r?\n|$)|(?s)id:\s*W5(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*transportation-enabled services\s*(?=\r?\n|$)|(?s)id:\s*W5(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*platform operations\s*(?=\r?\n|$)" "W5 should not keep older simulation method or platform/transportation-enabled domain wording."
