...
Assert-Contains "_sass/_base.scss" "(?s)\.paper-network-centrality-summary\s*\{[^}]*background:\s*linear-gradient\(135deg,\s*#750f6d,\s*#4b1359\)" "Overall network role summary should use the CUHK purple treatment."
Assert-Contains "_sass/_base.scss" "(?s)\.paper-network-centrality-summary\s*\{[^}]*border:\s*1px solid rgba\(196,\s*154,\s*44,\s*0\.48\)" "Overall network role summary should carry a gold accent frame."
