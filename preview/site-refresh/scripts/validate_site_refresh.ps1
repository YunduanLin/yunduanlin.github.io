...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*W2.*dual-network.*choice model.*network game.*equilibrium.*network centrality.*data-driven optimization.*facility location.*temporal retail.*scalable network operations.*\r?\n\s*-\s*SNAP\s*\r?\n\s*-\s*Gowalla" "W2 should use the revised concept, method, domain, and split dataset features."
...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*W3.*expert decision.*decision accuracy assessment.*machine learning.*algorithm design.*design science.*Kaggle.*adult census income.*hospital readmission rates" "W3 should use the revised decision, method, domain, and data features."
...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*J2.*network diffusion.*market dynamics.*data-driven optimization.*diffusion model.*user-generated content platform.*social media.*social network operations.*proprietary data from a short-video platform" "J2 should use the revised network diffusion, market dynamics, method, domain, and data features."
...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*J3.*network diffusion.*network structure.*network centrality.*market dynamics.*stochastic modeling.*fixed-point iteration.*network game.*equilibrium.*agent-based simulation.*social network operations.*scalable network operations.*network repository.*Facebook network" "J3 should use the revised network, market-dynamics, modeling, equilibrium, simulation, domain, and data features."
...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*C2.*physical activity monitoring.*wearable sensors.*machine learning.*time series data.*transfer learning.*healthcare analytics.*mobile health.*\r?\n\s*-\s*UCI\s*\r?\n\s*-\s*daily and sports activity dataset" "C2 should use the revised wearable-sensor, time-series, transfer-learning, and UCI dataset features."
