...
Assert-Contains "_data/paper_network.yml" "(?s)id:\s*C3.*agent skill.*skill optimization.*Monte Carlo tree search.*bilevel optimization.*agent-based simulation.*LLM-assisted optimization.*AI agents.*skill evolvement.*OR for AI" "C3 should use the revised agent-skill, optimization, simulation, and OR-for-AI features."
Assert-NotContains "_data/paper_network.yml" "(?s)id:\s*C3(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*agent skill design\s*(?=\r?\n|$)|(?s)id:\s*C3(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*platform decision support\s*(?=\r?\n|$)|(?s)id:\s*C3(?:(?!\r?\n  - id:).)*\r?\n\s*-\s*decision support\s*(?=\r?\n|$)" "C3 should not keep the older agent-skill-design, platform-decision-support, or decision-support feature wording."
